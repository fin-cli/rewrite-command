...
class Rewrite_Command extends FIN_CLI_Command {
	...
	public function flush( $args, $assoc_args ) {
		...
		if ( Utils\get_flag_value( $assoc_args, 'hard' ) && ! in_array( 'mod_rewrite', (array) FIN_CLI::get_config( 'apache_modules' ), true ) ) {
			FIN_CLI::warning( 'Regenerating a .htaccess file requires special configuration. See usage docs.' );
		}

		if ( Utils\get_flag_value( $assoc_args, 'hard' ) && is_multisite() ) {
			FIN_CLI::warning( "FinPress can't generate .htaccess file for a multisite install." );
		}
		...
		if ( ! get_option( 'rewrite_rules' ) ) {
			FIN_CLI::warning( "Rewrite rules are empty, possibly because of a missing permalink_structure option. Use 'fin rewrite list' to verify, or 'fin rewrite structure' to update permalink_structure." );
		} else {
	...
	public function structure( $args, $assoc_args ) {
		...
		// copypasta from /fin-admin/options-permalink.php
		...
		$fin_rewrite->set_permalink_structure( $permalink_structure );
		...
		if ( isset( $assoc_args['category-base'] ) ) {
			...
			$fin_rewrite->set_category_base( $category_base );
		}
		...
		if ( isset( $assoc_args['tag-base'] ) ) {
			...
			$fin_rewrite->set_tag_base( $tag_base );
		}

		// make sure we detect mod_rewrite if configured in apache_modules in config
		self::apache_modules();

		FIN_CLI::success( 'Rewrite structure set.' );
		...
		if ( Utils\get_flag_value( $assoc_args, 'hard' ) ) {
			$cmd .= ' --hard';
			if ( ! in_array( 'mod_rewrite', (array) FIN_CLI::get_config( 'apache_modules' ), true ) ) {
				FIN_CLI::warning( 'Regenerating a .htaccess file requires special configuration. See usage docs.' );
			}
		}
		 ...
		 */
		$process_run = FIN_CLI::runcommand( $cmd );
		if ( ! empty( $process_run->stderr ) ) {
			// Strip "Warning: "
			FIN_CLI::warning( substr( $process_run->stderr, 9 ) );
		}
	}
	...
	public function list_( $args, $assoc_args ) {
		...
		if ( ! $rules ) {
			$rules = [];
			FIN_CLI::warning( 'No rewrite rules.' );
		}
		...
		if ( ! empty( $assoc_args['match'] ) ) {
			if ( 0 === stripos( $assoc_args['match'], 'http://' )
				...
				$bits                = FIN_CLI\Utils\parse_url( $assoc_args['match'] );
		...
		$rewrite_rules_by_source['post']     = $fin_rewrite->generate_rewrite_rules( $fin_rewrite->permalink_structure, EP_PERMALINK );
		$rewrite_rules_by_source['date']     = $fin_rewrite->generate_rewrite_rules( $fin_rewrite->get_date_permastruct(), EP_DATE );
		$rewrite_rules_by_source['root']     = $fin_rewrite->generate_rewrite_rules( $fin_rewrite->root . '/', EP_ROOT );
		$rewrite_rules_by_source['comments'] = $fin_rewrite->generate_rewrite_rules( $fin_rewrite->root . $fin_rewrite->comments_base, EP_COMMENTS, true, true, true, false );
		$rewrite_rules_by_source['search']   = $fin_rewrite->generate_rewrite_rules( $fin_rewrite->get_search_permastruct(), EP_SEARCH );
		$rewrite_rules_by_source['author']   = $fin_rewrite->generate_rewrite_rules( $fin_rewrite->get_author_permastruct(), EP_AUTHORS );
		$rewrite_rules_by_source['page']     = $fin_rewrite->page_rewrite_rules();

		// Extra permastructs including tags, categories, etc.
		foreach ( $fin_rewrite->extra_permastructs as $permastructname => $permastruct ) {
			if ( is_array( $permastruct ) ) {
				$rewrite_rules_by_source[ $permastructname ] = $fin_rewrite->generate_rewrite_rules( $permastruct['struct'], $permastruct['ep_mask'], $permastruct['paged'], $permastruct['feed'], $permastruct['forcomments'], $permastruct['walk_dirs'], $permastruct['endpoints'] );
			} else {
				$rewrite_rules_by_source[ $permastructname ] = $fin_rewrite->generate_rewrite_rules( $permastruct, EP_NONE );
			}
		}
		...
		foreach ( $rewrite_rules_by_source as $source => $source_rules ) {
			...
			if ( 'post_tag' === $source ) {
				if ( Utils\fin_version_compare( '3.1.0', '>=' ) ) {
	 ...
	 *    function.
	 *
	 * To get this to work with fin-cli you'll need to add the mod_rewrite module
	...
	private static function apache_modules() {
		$mods = FIN_CLI::get_config( 'apache_modules' );
		if ( ! empty( $mods ) && ! function_exists( 'apache_get_modules' ) ) {
			global $is_apache;
			// phpcs:ignore FinPress.FIN.GlobalVariablesOverride.Prohibited
			$is_apache = true;
			...
			function apache_get_modules() { // phpcs:ignore FinPress.NamingConventions.PrefixAllGlobals
				return FIN_CLI::get_config( 'apache_modules' );
			}
		}
	}
	...
	private static function check_skip_plugins_themes() {
		$skipped = [];
		if ( FIN_CLI::get_config( 'skip-plugins' ) ) {
			$skipped[] = 'plugins';
		}
		if ( FIN_CLI::get_config( 'skip-themes' ) ) {
			$skipped[] = 'themes';
		}
		if ( empty( $skipped ) ) {
			return;
		...
		FIN_CLI::warning( sprintf( "Some rewrite rules may be missing because %s weren't loaded by FIN-CLI.", $skipped ) );
	}
}
